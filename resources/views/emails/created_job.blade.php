...
<body style="margin: 20px;">
	<h1 style="text-align: center;">Đây là email thông báo có đơn hàng mới được khởi tạo</h1>
		...
		<div style="font-size: 16px; line-height: 25px;" class="job-component">
			<div style="width: 77%; float: left; margin-left: 3%;">
				<div style="font-size: 26px; font-weight: bold; color:#ff00a3; margin-bottom: 5px;">
					@if($order->user){{ $order->user->name }} @else echo "No name"; @endif
				</div>
				<div style="font-size: 26px; font-weight: bold; margin-bottom: 5px;">
					<span style="text-transform: capitalize;">{{ $order->address }}</span>
				</div>
				<div style="">
					Số tiền: <span style="font-size:20px; font-weight: bold; color:red;">{{ number_format($order->price, 0) }} vnd</span>
				</div>
				<div style="">
					Chi tiết đơn hàng:
